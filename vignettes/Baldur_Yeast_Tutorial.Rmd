...
# Since baldur itself does not deal with missing data we filter out
# rows that have missing data for the purpose of the tutorial.
# Else, one would first need to run imputation, then the tutorial follows
# in the same fashion.
yeast_norm <- yeast %>% 
  # Remove missing data
  tidyr::drop_na() %>% 
...
First we will setup the needed variables for using `baldur` without partioning the data.
Then, partitioning and setting up `baldur` after trend-partioning.
```{r cluster}
...
yeast_cluster <- yeast_norm %>% 
  trend_partitioning(
    ...
    formula = sd ~ mean + c, # Set the formula you want to use for the partitioning
                             ...
                             # if e.g., there seems to be a different slope as wee
