...
estimate_gamma_hyperparameters.lgmr <- function(reg, data){
  ...
  check_order <- any(data$mean != reg$data$mean)
  if (check_order) {
    ori_order <- data$mean
    data <- data[match(reg$data$mean, data$mean),]
  }
  ...
  data <- data %>%
    dplyr::mutate(
      ...
      beta  = alpha / mu_fun(pars$theta, pars$coef, mean, m, s)
    )

  if (check_order) {
    data <- data[match(ori_order, reg$data$mean),]
  }
